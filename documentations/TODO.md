...
- [~] Provision a separate staging Mongo database and credentials with no production write authority where infrastructure supports it; never reuse production application credentials. Preview resolves the separate staging database through its own `DB_NAME`, and runtime/API smoke proves data separation; credential-level denial of production writes is not yet evidenced.
...
- [x] Define fixture/anonymization and production-data protection rules; no casual production DB write or clone. The initial bounded fixture copied only the four approved users, all services/public projects, one company profile, current testimonials and required CMS pages; tokens were cleared and notification preferences disabled.

**Progress evidence (2026-09-10):** The existing Vercel project `dm-develon` now deploys the Git branch `staging` as a protected Preview and binds it to `https://staging.dmdevelon.website`; the obsolete `dm-develon-staging.vercel.app` origin is not canonical or used by the fixture workflow. Preview has staging-scoped `APP_ENV`, origins/CORS, JWT, separate `DB_NAME` and `ALLOW_DB_SEED=false`. Email, push, Cloudinary and cron credentials were intentionally not copied. Home and the public services/projects/testimonials/company-profile/CMS APIs return 200; `POST /api/seed` returns 404.

**Fixture evidence (2026-09-10):** A one-use, digest-locked and `APP_ENV=staging`-only route imported exactly 4 approved users (Milan Drazic as admin; Gordana, Sanja and Klaudija as users), 10 services, 8 public projects, 1 company profile, 3 testimonials and 4 required CMS pages. The initial run inserted the exact set, the second run matched every record with zero inserts/modifications, and unique indexes exist for user email plus project/CMS slug. Notifications, chat/read data, project messages and audit/history were not copied and their staging counts were verified as zero. Production counts/content and all four password hashes were checked after import; Milan's production `updatedAt` changed concurrently while his core content/hash remained equal. The temporary route and local payload were removed after verification. Credential-level Mongo isolation, provider boundaries and authenticated browser login smoke remain open; DMD-FND-2 has not started.

**Branch status (2026-09-10):** The Git branch `staging` exists from the current `main` baseline and is the branch for all further foundation work. The canonical domain is permanently bound to this branch on the `dm-develon` project. The obsolete `dm-develon-staging` Vercel project still deploys pushes and must be disabled or filtered separately; it is not used as the canonical staging target.
