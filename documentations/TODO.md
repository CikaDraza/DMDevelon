...
**Branch status (2026-09-10):** The Git branch `staging` now exists from the current `main` baseline and is the branch for all further foundation work. `ALLOW_DB_SEED=false` is explicitly configured for Preview. The canonical domain currently points to a manually verified deployment; permanent `staging` branch-domain binding remains a Vercel project-setting step.
