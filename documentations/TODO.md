...
- [~] Provision a separate staging Mongo database and credentials with no production write authority where infrastructure supports it; never reuse production application credentials. Preview resolves the separate `staging-portfolio_db` database through its own `DB_NAME`, but the 2026-09-11 re-audit proves the effective branch configuration still resolves the same `MONGO_URL` as Production and exposes production write-capable privileges. A separately provisioned credential is not complete until it is assigned to the `staging` Preview branch and the safe authority audit passes.
