...
- [~] Isolate Cloudinary at provider/account credential level where available. Otherwise use an explicit staging namespace plus a fail-closed application guard and scoped credentials; a folder prefix with an unrestricted production credential is not physical isolation. Preview credentials are currently absent, so staging Cloudinary is safely disabled rather than operationally isolated/smoke-tested.
- [~] Restrict email and push to explicit safe staging recipients/test providers, and disable or safely isolate scheduled cron/job side effects. Exact recipient allowlisting and explicit staging-cron opt-in exist in code; Preview provider/cron credentials are currently absent and live delivery smoke is still open.
...
- [ ] Before any future production deployment, restore the required production-only Cloudinary, Resend, VAPID and cron variables without restoring them to Preview; the currently running production deployment retains its immutable env snapshot, but the project configuration for a new production build no longer lists those provider variables.
