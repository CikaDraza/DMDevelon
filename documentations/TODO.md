...
**Status:** Foundation execution in progress. DMD-FND-0 is complete; DMD-FND-1 remains open on the concrete isolation and live-smoke blockers recorded below. DMD-FND-2 has not started. This is the execution index, while documents under documentations/new-business-model/ remain the architecture and product contracts.
...
- [~] Add a fail-closed configuration guard: when APP_ENV=staging, reject known production Mongo/resource identities before the application can serve traffic or run a job. The repository guard, negative tests and canonical-origin enforcement exist; complete guarded staging verification remains gated by the staging-only credential/provider boundary and authenticated smoke.
...
- [~] Restrict email and push to explicit safe staging recipients/test providers, and disable or safely isolate scheduled cron/job side effects. Separate branch-scoped Resend, VAPID and cron credentials now exist; exact recipient allowlisting and explicit staging-cron opt-in are configured. Key-shape/public-private VAPID configuration and unauthorized cron behavior are verified, but live email, push, successful cron execution and authenticated application smoke still await a new safe staging deployment.
...
**Authenticated user identity regression (2026-09-11):** The production `PUT /api/users/undefined` incident was traced to `/api/auth/me` replacing the cached login payload with a raw user shape that lacked the canonical `id`, while token-derived project APIs continued to work. `/api/auth/me` now returns the same `authUserPayload` contract as register, login and refresh, including the notification preferences consumed by settings. Client identity resolution is centralized as `user?.id ?? user?._id`; avatar, profile update, account deletion and testimonial ownership use that result, and identity absence fails before avatar file encoding or `/api/upload`, preventing a new orphan asset. Settings remains token-scoped and required no ID-bound route change. Cloudinary code and credentials were not changed. Local verification passed the focused auth/API regressions, `npm test` (8/8 files), `npm run test:ui` (50/50 assertions), `npm run test:api` (12/12 files, 236/236 assertions), `npm run typecheck` and the production build. Staging deployment and authenticated smoke evidence are still required before this item can be considered deployed; no production deployment is authorized by this evidence.
...
- [ ] Capture the React 18 login observation that one login/page mount currently produces approximately 17 successful `GET /api/auth/me` requests in development. Treat it as a known client-auth ownership/performance gap, not as 17 server-side retries or a completed regression baseline.
