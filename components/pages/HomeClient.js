...
const PrevArrow = ({ onClick, className }) => (
  <button
    onClick={onClick}
    className={`absolute left-0 top-1/2 -translate-y-1/2 z-10 p-3 bg-[#FFB633] rounded-full hover:bg-[#e5a32e] transition-colors -translate-x-1/2 lg:-translate-x-6 ${className}`}
  >
...
const NextArrow = ({ onClick, className }) => (
  <button
    onClick={onClick}
    className={`absolute right-0 top-1/2 -translate-y-1/2 z-10 p-3 bg-[#FFB633] rounded-full hover:bg-[#e5a32e] transition-colors translate-x-1/2 lg:translate-x-6 ${className}`}
  >
    <ChevronRight className="w-6 h-6 text-black" />
  </button>
);

// Custom Dots with active state styling
const CustomDots = ({ dots, activeIndex, onDotClick }) => (
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled
          ? "bg-[#0f0f10]/90 backdrop-blur-lg shadow-lg pt-5"
          : "bg-transparent"
      }`}
    >
        ...
        <div className="flex items-center justify-between w-full">
          ...
          <a href="/" className="flex items-center gap-3">
            ...
            <div className="w-auto flex flex-wrap">
              <span className="w-full font-logo lg:text-xl font-bold text-white tracking-wider">
                DMDevelon
              </span>
        ...
        <AnimatePresence>
          {mobileMenuOpen && (
            <motion.nav
              ...
              className="md:hidden mt-4 pb-4 border-t border-white/10 bg-[#0f0f10]/90 backdrop-blur-lg shadow-lg"
            >
  ...
  return (
    <section id="hero" className="w-full relative overflow-hidden pt-20">
      <div className="container mx-auto px-4 h-full relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-7 gap-4 lg:gap-0 min-h-[calc(100vh-80px)] items-stretch relative">
          {/* Left Column */}
          <motion.div
            initial="hidden"
            animate="visible"
            variants={fadeInLeft}
            className="lg:col-span-2 bg-transparent rounded-l-2xl rounded-r-2xl lg:rounded-r-none rounded-t-2xl lg:rounded-t-none lg:rounded-tl-2xl lg:rounded-bl-2xl flex flex-col justify-center p-8 lg:p-12 relative overflow-hidden"
          >
            <motion.span
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              transition={{ delay: 0.3 }}
              className="text-white text-sm font-semibold tracking-widest mb-4"
            >
              WELCOME
            </motion.span>
          ...
          <motion.div
            ...
            animate="visible"
            variants={fadeInLeft}
            className="w-full h-10 lg:col-span-7 rounded-l-2xl lg:rounded-r-none rounded-t-2xl lg:rounded-t-none lg:rounded-tl-2xl lg:rounded-bl-2xl flex flex-col justify-center p-8 lg:p-12 absolute left-0 z-10"
          >
            <motion.div
              initial="hidden"
              animate="visible"
              variants={bounceIn}
              className="absolute w-full right-0 top-4 lg:top-6 z-20 flex justify-end"
            >
...
function ProjectsSection({ projects }) {
  ...
  const colorMap = {
    ...
    teal: "bg-teal-600",
    sky: "oklch(58.8% 0.158 241.966)",
    violet: "oklch(54.1% 0.281 293.009)",
    emerald: "oklch(43.2% 0.095 166.913)",
  ...
  const settings = {
    dots: true,
    infinite: displayProjects.length > 3, // Only infinite if enough items
    speed: 500,
    slidesToShow: 3,
    slidesToScroll: 1,
    arrows: true,
    ...
    appendDots: (dots) => (
      <CustomDots
        ...
        onDotClick={(index) => sliderRef?.slickGoTo(index)}
      />
    ),
    customPaging: () => <div />, // Empty div, we use appendDots
    responsive: [
      {
        // Laptop (1024px - 1279px) - 2 cards
        breakpoint: 1279,
        settings: {
          slidesToShow: 2,
          slidesToScroll: 1,
          infinite: displayProjects.length > 2,
          arrows: true,
        },
      },
      {
        // Tablet and below - 1 card, infinite
        breakpoint: 767,
        settings: {
          slidesToShow: 1,
          slidesToScroll: 1,
            ...
            {displayProjects.map((project) => (
              <div key={project._id} className="px-2 md:px-3 p-4">
                <motion.div
                  ...
                  className={`${colorMap[project.color] || "bg-blue-600"} rounded-2xl overflow-hidden group h-full`}
                >
...
function TestimonialsSection({ testimonials }) {
  ...
  const defaultTestimonials = [
    ...
    },
    {
      ...
      rating: 5,
      comment:
        "Milan is a true professional. His attention to detail and creative solutions are outstanding.",
  ...
  const settings = {
    dots: true,
    infinite: displayTestimonials.length > 3,
    speed: 500,
    slidesToShow: 3,
    slidesToScroll: 1,
    arrows: true,
    ...
    responsive: [
      {
        ...
        settings: {
          slidesToShow: 2,
          slidesToScroll: 1,
          infinite: displayTestimonials.length > 2,
          arrows: true,
        },
      },
      {
  ...
  return (
    <section id="testimonials" className="section-full py-20 bg-[#0a0a0b]">
        ...
        <div className="hidden lg:block relative px-4 md:px-12 lg:px-16">
          ...
          >
            {displayTestimonials.map((testimonial) => (
              <div key={testimonial._id} className="px-2 md:px-3 h-full pt-4">
                <motion.div
                  whileHover={{ y: -5 }}
                  className="glass rounded-2xl p-6 h-full border border-white/10 bg-white/5 backdrop-blur-sm flex flex-col"
                >
                  {/* Stars - fixed at top */}
      ...
      <style>{`
        ...
        /* Ensure arrows don't cause overflow */
