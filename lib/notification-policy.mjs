...
export function resolveDeliveryChannels({
  ...
  // Someone with the app open will see the toast and the bell immediately;
  // emailing and pushing them as well is pure noise.
  if (recipientOnline) {
  ...
  return {
    inApp: true,
    email: millisSince(lastEmailedAt, now) >= throttleMs,
