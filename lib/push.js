...
import { assertSafeStagingRecipient } from "./staging-safety.mjs";
...
export async function sendPushToUser(userId, payload = {}) {
  ...
  if (process.env.APP_ENV === "staging") {
    try {
      const recipient = await User.findById(userId).select("email");
      assertSafeStagingRecipient(recipient?.email);
    } catch (error) {
      console.error("push: staging recipient policy blocked delivery:", error.message);
      return { sent: 0, failed: 0, pruned: 0, skipped: "unsafe_recipient" };
    }
  }
  ...
  await Promise.all(
    subs.map(async (sub) => {
      try {
        ...
        // 404/410 — subscription expired or was unsubscribed.
        // 401/403 — the push service rejected our VAPID credentials for THIS
        // subscription, which in practice means it was created against an
        // older key. Pruning it is what lets the client rebuild it against the
        // current one; left in place it fails on every send forever, which is
        // the classic "push worked and then silently stopped".
        if (status === 404 || status === 410 || status === 401 || status === 403) {
          dead.push(sub._id);
          if (status === 401 || status === 403) {
            console.warn(
              `push: ${status} for a subscription — dropping it so the device re-subscribes with the current VAPID key`,
            );
          }
        } else {
          failed++;
