...
export const emailTemplates = {
  ...
  newMessageDigest: (data) => {
    ...
    const showFooterWordmark = data.wordmarkUrl && data.wordmarkUrl !== data.logoUrl;
    const subject =
      total === 1
  ...
  // 7. Poziv da se pridruži projektu kao saradnik (collaborator/viewer)
  //   data: { inviterName, projectTitle, roleLabel, inviteUrl, recipientEmail,
  //           expiresAt, personalMessage }
  //   recipientEmail is the REAL address (this goes straight to their inbox,
  //   unlike the /invite preview page which shows a masked hint to anyone
  //   holding the link before they've proven who they are).
  projectInvite: (data) => {
    const expiresLabel = data.expiresAt
      ...
      : "";
    return {
      subject: `${data.inviterName} invited you to ${data.projectTitle} on DMDevelon`,
      html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #FFB633;">You've been invited</h2>
        <p><strong>${data.inviterName}</strong> invited you to collaborate on
           <strong>${data.projectTitle}</strong> as a <strong>${data.roleLabel}</strong>.</p>
        ${
          data.personalMessage
            ? `<p style="background: #f5f5f5; padding: 12px 16px; border-radius: 8px; white-space: pre-wrap;">${data.personalMessage}</p>`
            : ""
        }
        ...
        <p style="color: #666; font-size: 14px;">
          Or copy this link: ${data.inviteUrl}
        </p>
        <p style="color: #999; font-size: 12px;">
          This invitation is for ${data.recipientEmail || "your email address"}
          ${expiresLabel ? `and expires on ${expiresLabel}` : "and will expire in 7 days"}.
