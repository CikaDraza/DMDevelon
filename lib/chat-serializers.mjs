...
export function serializeChannelSummary(
  ...
  const out = {
    ...
    // Gates the header's purge menu. Same key the /purge endpoint checks, so a
    // hidden menu and a 403 can never disagree about who may wipe a channel.
    canModerate: Boolean(accessObj?.permissions?.messagesModerate),
