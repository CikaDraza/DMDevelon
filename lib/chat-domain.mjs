...
/**
 * Validate a purge request and resolve it to one absolute cutoff.
 *
 * `now` is injectable so the cutoff is a pure function of its inputs — the
 * caller then audits and queries with the SAME Date instance, instead of one
 * value deciding what to delete and a second, slightly later one describing it.
 *
