...
export function assertSafeStagingRecipient(recipient, env = process.env) {
  ...
  if (!recipients.length || recipients.some((email) => !allowed.has(email))) {
    throw new Error("Staging outbound delivery blocked for a non-safe recipient");
  }
}
...
export function validateStagingRuntimeConfig(env = process.env) {
  ...
  if (!env.STAGING_DB_NAME) {
    throw new Error("Staging requires STAGING_DB_NAME as its expected identity");
  }
  if (env.DB_NAME !== env.STAGING_DB_NAME) {
    throw new Error("DB_NAME does not match the expected staging database");
  }
  ...
  if (cloudinaryConfigured) {
    if (
      !env.STAGING_CLOUDINARY_NAME ||
      env.CLOUDINARY_NAME !== env.STAGING_CLOUDINARY_NAME
    ) {
      throw new Error("Cloudinary does not match its staging provider identity");
    }
    ...
    if (!/^staging(?:\/|$)/.test(env.CLOUDINARY_FOLDER || "")) {
      throw new Error("Staging Cloudinary requires a staging/ folder namespace");
    }
  }

  const pushValues = [env.VAPID_PUBLIC_KEY, env.VAPID_PRIVATE_KEY];
  ...
  const outboundConfigured = Boolean(
    env.RESEND_API_KEY || pushValues.some(Boolean),
  );
  if (outboundConfigured && safeStagingRecipients(env).size === 0) {
    throw new Error("Staging outbound delivery requires STAGING_SAFE_RECIPIENTS");
  }
