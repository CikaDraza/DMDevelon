 ...
 */
export function preparePhaseArchive(proposal, projectMilestones = [], { force = false } = {}) {
  ...
  // Started work is protected by default: an accepted phase someone has
  // begun is a record of what was agreed AND done, and it is visible to the
  // client. `force` is the operator deliberately overriding that — a mistake
  // caught late, or scope both sides later agreed to drop. It never widens
  // WHAT may be removed (the master-proposal rule above still applies), only
  // whether started work blocks it; the caller is responsible for demanding a
  // reason and an explicit confirmation before passing it.
  if (startedMilestones.length > 0 && !force) {
    throw new ProposalStateError(
  ...
  return {
    ...
    // True when the override actually discarded work in progress — the caller
    // uses this to word the audit trail differently.
    forcedOverStartedWork: startedMilestones.length > 0,
    startedMilestoneCount: startedMilestones.length,
  };
}
